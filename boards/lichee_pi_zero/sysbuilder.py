...
class Bootloader(object):
    def __init__(self, workdir):
        ...
        self._own_workdir = str(Path(workdir).joinpath('uboot'))
        self._dir = None
    ...
    def fetch(self, path):
        ...
        # download sources
        archive_filename = str(Path(self._workdir).joinpath('uboot.tar.gz'))
        if not Path(archive_filename).is_file():
            wget('-O{}'.format(archive_filename), path, _out=sys.stdout, _err=sys.stderr)

        # extract sources
        extracted_dir = str(Path(self._own_workdir).joinpath('_extracted'))
        mkdir('-p', extracted_dir)
        tar('-xf', archive_filename, '-C', extracted_dir, _out=sys.stdout, _err=sys.stderr)
        
        uboot_src_dir = [d for d in Path(extracted_dir).iterdir() if d.is_dir()]
        if (len(uboot_src_dir) != 1):
            raise RuntimeError('U-boot sources failed to extract')

        self._dir = uboot_src_dir[0]

    def build(self, arch, cc, config, cpus=8):
        dirname = self._dir
        make('-C', dirname, 'ARCH={}'.format(arch), 'CROSS_COMPILE={}'.format(cc), config, _out=sys.stdout)
        make('-C', dirname, 'ARCH={}'.format(arch), 'CROSS_COMPILE={}'.format(cc), '-j{}'.format(cpus), _out=sys.stdout)


class Kernel(object):
    def __init__(self, workdir):
        self._workdir = workdir
        self._own_workdir = str(Path(workdir).joinpath('kernel'))
        self._dir = None
    ...
    def fetch(self, path):
        ...
        kernel_src_dir = [d for d in Path(extracted_dir).iterdir() if d.is_dir()]
...
class RootFS(object):
    def __init__(self, workdir):
        self._workdir = workdir
        self._own_workdir = str(Path(workdir).joinpath('rootfs'))
        self._dir = None
    ...
    def fetch(self, path):
        ...
        # download sources
        project_dir = str(Path(self._own_workdir).joinpath('busybox'))
        if not Path(project_dir).is_dir():
            git('clone', '--depth=1', 'git://git.busybox.net/busybox', project_dir, _out=sys.stdout)

        self._dir = project_dir

    def build(self, arch, cc, config, cpus=8):
        dirname = self._dir
        install_dir = str(Path(dirname).joinpath('_install'))

        if Path(config).is_file():
            print('use own config')
            cp(config, str(Path(self._dir).joinpath('.config')))
        else:
            make('-C', dirname, 'ARCH={}'.format(arch), 'CROSS_COMPILE={}'.format(cc), config, _out=sys.stdout)

        make('-C', dirname, 'ARCH={}'.format(arch), 'CROSS_COMPILE={}'.format(cc), '-j{}'.format(cpus), _out=sys.stdout)
        make('-C', dirname, 'ARCH={}'.format(arch), 'CROSS_COMPILE=arm-linux-gnueabihf-', 'install', 'CONFIG_PREFIX={}'.format(install_dir), _out=sys.stdout)
...
def LicheePiZero_RootFS(workdir):
    ...
    rootfs.fetch('')
    # TODO: change config
    rootfs.build(ARCH, 'arm-linux-gnueabihf-', 'defconfig')
...
def LicheePiImage(workdir, boot_files, kernel_files, rootfs_files):
    ...
    mkimage('-C', 'none', '-A', 'arm', '-T', 'script', '-d', 'boot.cmd', '/tmp/p1/boot.scr')
